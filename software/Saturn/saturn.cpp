...
void UpdateButtons()
{
    ...
    if(hw.switches[Soundsketch::FOOTSWITCH_1].TimeHeldMs() >= 700 && !save_check && !expression_pressed && hw.switches[Soundsketch::FOOTSWITCH_2].TimeHeldMs() <= 50) 
    {
        Save();
        save_check = true;
    }
...
int main(void)
{
    ...
    predelay.Init(hw.knob[Soundsketch::KNOB_1], 0.0f, 1.0f, Parameter::LINEAR);
    mix.Init(hw.knob[Soundsketch::KNOB_2], 0.0f, 1.0f, Parameter::LINEAR);
    filter.Init(hw.knob[Soundsketch::KNOB_3], 0.0f, 1.0f, Parameter::LINEAR); 
    delay_time.Init(hw.knob[Soundsketch::KNOB_4], 0.0f, 1.0f, Parameter::LINEAR);
    delay_fdbk.Init(hw.knob[Soundsketch::KNOB_5], 0.0f, 1.0f, Parameter::LINEAR);
    mod.Init(hw.knob[Soundsketch::KNOB_6], 0.0f, 1.0f, Parameter::LINEAR);
