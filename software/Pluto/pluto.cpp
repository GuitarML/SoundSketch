...
// Allows for Stereo In/Out, 6 knobs, 3 3-way switches,  2 SPST Footswitches, 2 LEDs.
//
...
void UpdateButtons()
{
    ...
    if (hw.switches[Soundsketch::FOOTSWITCH_1].RisingEdge())
    {
        if (!pausePlaybackA) {
            looperA.TrigRecord();
            isPlaybackA = false;
    ...
    if(hw.switches[Soundsketch::FOOTSWITCH_1].TimeHeldMs() >= 1000)
    {
    ...
    if ((hw.switches[Soundsketch::FOOTSWITCH_2].RisingEdge()) || (hw.switches[Soundsketch::FOOTSWITCH_1].RisingEdge()) ) // TODO, verify this logic runs both loopers in the same way when in stereo mode
    {
        if (!pausePlaybackB) {
            looperB.TrigRecord();
            isPlaybackB = false;
    ...
    if((hw.switches[Soundsketch::FOOTSWITCH_2].TimeHeldMs() >= 1000) || (hw.switches[Soundsketch::FOOTSWITCH_1].TimeHeldMs() >= 1000))
    {
...
int main(void)
{
    ...
    levelA.Init(hw.knob[Soundsketch::KNOB_1], 0.0f, 1.0f, Parameter::LINEAR);
    modA.Init(hw.knob[Soundsketch::KNOB_2], 0.0f, 1.0f, Parameter::LINEAR);
    levelB.Init(hw.knob[Soundsketch::KNOB_3], 0.0f, 1.0f, Parameter::LINEAR); 
    speedA.Init(hw.knob[Soundsketch::KNOB_4], 0.0f, 1.0f, Parameter::LINEAR);
    modB.Init(hw.knob[Soundsketch::KNOB_5], 0.0f, 1.0f, Parameter::LINEAR);
    speedB.Init(hw.knob[Soundsketch::KNOB_6], 0.0f, 1.0f, Parameter::LINEAR); 
