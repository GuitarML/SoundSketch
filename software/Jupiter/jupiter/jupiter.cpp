                          ...
                          size_t                    size)
{
    ...
    for (size_t i = 0; i < size; i++)
    {
        ...
        if(!bypass) {
            ...
            // MISO by default (TODO maybe add stereo option)
            inputL = in[0][i];
            inputR = in[0][i];
...
int main(void)
{
    ...
    rdecay.Init(hw.knob[Soundsketch::KNOB_1], 0.0f, 1.0f, ::daisy::Parameter::LINEAR); // LOG to go high fast?
    lowfreq.Init(hw.knob[Soundsketch::KNOB_2], 0.0f, 1.0f, ::daisy::Parameter::LINEAR);
    highfreq.Init(hw.knob[Soundsketch::KNOB_3], 0.0f, 1.0f, ::daisy::Parameter::LINEAR);
    mix.Init(hw.knob[Soundsketch::KNOB_4], 0.0f, 1.0f, ::daisy::Parameter::LINEAR);
    lowshelf.Init(hw.knob[Soundsketch::KNOB_5], 0.0f, 1.0f, ::daisy::Parameter::LINEAR);
    highshelf.Init(hw.knob[Soundsketch::KNOB_6], 0.0f, 1.0f, ::daisy::Parameter::LINEAR);
